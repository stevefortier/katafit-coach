...
Read-only member tabs show currently authorized retained Coach feed interactions. For chief-managed dojo Coaches, access follows the member’s existing Dojo Chief sharing settings in Kata.fit—not a separate external-agent permission. The backend verifies current chief, credential and membership authority and filters content by sharing. Protect installation credentials as chief access. Member content is never automatically added to operator AI context. Attachment contents and nested workout conversations are outside this initial view.
