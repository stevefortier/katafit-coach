...
1. Create a connection credential in Kata.fit's external Coach settings for the intended identity/scope. Paste it in the studio. Installation-local Dojo MCP configuration is available only to the authenticated Studio admin; it is not a Kata.fit app-managed registration. See [local MCP transport, isolation and risks](docs/local-dojo-mcp.md).
