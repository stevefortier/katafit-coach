...
A local Node service with a browser studio for your Kata.fit connection, an explicit model provider, and a versioned coaching persona. No Hermes or OpenClaw dependency. One installation, one credential-authorized personal or Dojo scope. No browser-triggered shell, proactive scheduler, or marketplace. Installation-local Dojo MCP tools may mutate their own external services when the model selects them.
...
1. Create a connection credential in Kata.fit's external Coach settings for the intended identity/scope. Paste it in the studio. Installation-local Dojo MCP registrations can be saved by the authenticated Studio admin, but **runtime is disabled**: no local tools are discovered or invoked until backend chief/Dojo and per-dispatch lease authority plus mutation receipts exist. It is not a Kata.fit app-managed registration. See [the blocked contract](docs/local-dojo-mcp.md).
