...
Read-only member tabs show currently authorized retained Coach feed interactions. For chief-managed dojo Coaches, messages **to and from Coach** are baseline leader-readable through dojo membership—not gated on all five category-sharing settings or a separate external-agent permission. Activity records still follow the member’s existing per-category Dojo Chief sharing settings. The backend verifies current chief, credential and membership authority. Protect installation credentials as chief access. Member content is never automatically added to operator AI context. Retained main and nested workout messages require the companion membership-conversation backend; raw attachment contents remain excluded. Upgrade the backend and standalone installation separately; this UI change alone does not expand backend reads.
