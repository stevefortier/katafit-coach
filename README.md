...
**Single-user Linux pilot, not production-certified.** Negotiated v2 adds request-scoped read tools and opt-in original-image input; v1 remains text-only. Packed v2 proof uses real Pi, final-backend MCP and disposable MongoDB with synthetic users/storage, both a deterministic provider and an authorized live vision model. Two live runs completed; a third failed closed at the model budget. Prior text-only proof remains historical. No customer data or production credentials were used. See [request data access and proof](docs/request-data-access.md) and [bounded live results and limits](docs/evidence/data-final-integration.md).
