...
Entries contain UTC time, severity, fixed source/stage, optional randomly generated local operation reference, fixed safe error code/hint and allowlisted nonnegative integer metadata. References correlate stages within an operation but are **not backend request/user/member IDs**. The one content exception is a typed-task `task-output-correction` entry: after credential screening, a JSON/schema/semantic rejection retains the full candidate (up to the existing 24,000-byte output limit), exact parser/schema/semantic reason, kind and attempt number. A disclosure in the viewer keeps that private text collapsed by default. `TASK_OUTPUT_SECURITY` and `TASK_OUTPUT_SIZE` never retain raw text; neither do ordinary provider/chat failures. Do not share exported JSON casually: a rejected candidate may contain private meal or health data. State changes are logged; repeated idle polling is not. Last error is independent of live worker state and remains visible through subsequent idle polls. Cancellation is a warning and does not overwrite the last error.
