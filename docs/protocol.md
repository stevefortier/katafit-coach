...
- Scope/requester generation fencing, Clear semantics, future/backdated exclusion, owner sharing and audience filtering live in the backend. The worker cannot establish them independently from the current opaque context. A backend contract test against a disposable real app DB is still needed; synthetic fixtures do not prove production enforcement.
- Current backend Dojo hydration shown in reference is requester-oriented and may exclude otherwise owner-authorized peer data. Owner-setting/audience-aware shared-context expansion belongs in the app, never a client workaround.
